...
int main(int argc, char *argv[])
{
    ...
    timestamp_t  t0, t1, usecs, tNeighbourStart, tNeighbourEnd, tComputeStart,
        ...
        tWriteCompStart, tWriteCompEnd, tInitStart, tInitEnd, tLoopStart,
        tLoopEnd;
    ...
    context =
        clCreateContext(NULL, 1, &devices[device_id], NULL, NULL, &status);
    if (status != CL_SUCCESS)
    {
    ...
    t_cellVDendPtr = clCreateImage(context, CL_MEM_READ_WRITE, &format,
    ...
    //-----------------------------------------------------
    status = clEnqueueWriteBuffer(cmdQueue, bufferCellState, CL_FALSE, 0,
                                  2 * IO_NETWORK_SIZE * STATE_SIZE *
                                      sizeof(cl_double),
                                  cellStatePtr, 0, NULL, &writeDone);

    status |=
        clEnqueueWriteImage(cmdQueue, t_cellVDendPtr, CL_FALSE, origin, region,
                            0, 0, cellVDendPtr, 0, NULL, &writeDone);
    ...
    **/
    ...
    cl_program neighbourProgram = clCreateProgramWithSource(
        context, 1, (const char **)&neighbourBuffer, &neighbourSize, &status);
    free(neighbourBuffer);

    cl_program computeProgram = clCreateProgramWithSource(
        context, 1, (const char **)&computeBuffer, &computeSize, &status);
    free(computeBuffer);
    ...
    if (status != CL_SUCCESS)
    {
        printf("error in step 7, neighbourProgram, error msg: %s\n",
               getErrorString(status));
        getLog(neighbourProgram, devices[device_id]);
        exit(-1);
    }
    status = clBuildProgram(computeProgram, 1, &devices[device_id], options,
                            NULL, NULL);
    if (status != CL_SUCCESS)
    {
    ...
    neighbourKernel =
        clCreateKernel(neighbourProgram, "neighbour_kernel", &status);
    if (status != CL_SUCCESS)
    {
    ...
    //-----------------------------------------------------
    status =
        clSetKernelArg(neighbourKernel, 0, sizeof(cl_mem), &t_cellVDendPtr);

    status |= clSetKernelArg(neighbourKernel, 1, sizeof(cl_mem),
                             &bufferCellCompParams);

    status |= clSetKernelArg(computeKernel, 0, sizeof(cl_mem), &t_cellVDendPtr);

    status |=
        clSetKernelArg(computeKernel, 1, sizeof(cl_mem), &bufferCellState);

    status |=
        clSetKernelArg(computeKernel, 2, sizeof(cl_mem), &bufferCellCompParams);
    ...
    for (i = 0; i < simSteps; i++)
    {
        ...
        status = clSetKernelArg(neighbourKernel, 2, sizeof(cl_uint), &i);
        status |= clSetKernelArg(computeKernel, 3, sizeof(cl_mod_prec), &iApp);
        status |= clSetKernelArg(computeKernel, 4, sizeof(cl_uint), &i);
        if (status != CL_SUCCESS)
        {
            printf("error in step 9\n");
            exit(-1);
        ...
        //-----------------------------------------------------
        status = clEnqueueNDRangeKernel(cmdQueue, neighbourKernel, 2, NULL,
        ...
        status = clEnqueueNDRangeKernel(
            cmdQueue, computeKernel, 2, NULL, globalWorkSize, NULL, 1,
            ...
            else
            {
              ...
              fputs(temp, pOutFile);
                ...
                if (EXTRA_TIMING)
                {
                    tWriteFileEnd = get_timestamp();
                    tWriteFile += (tWriteFileEnd - tWriteFileStart);
                    // DEBUG_PRINT(("twritestart: \t\t %lld | twriteend: \t\t %lld | twritediff: \t\t %lld \n", tWriteFileStart, tWriteFileEnd, tWriteFileEnd - tWriteFileStart));
		            }
            }
        }
    }
    ...
    usecs = (t1 - t0); // / 1000000;
    DEBUG_PRINT(
        ("%d ms of brain time in %d simulation steps\n", simTime, simSteps));
